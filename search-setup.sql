...
create index if not exists rambam_embeddings_ivfflat
  on rambam_embeddings using ivfflat (embedding vector_cosine_ops) with (lists = 50);
...
returns table (
  id int, book_number int, book_name text, treatise_name text,
  chapter_number int, content_text text, content_summary text,
  sefaria_ref text, similarity float
)
language plpgsql
as $$
begin
  return query
  select re.id, re.book_number, re.book_name, re.treatise_name,
         re.chapter_number, re.content_text, re.content_summary, re.sefaria_ref,
...
create table if not exists rambam_search_log (
  ...
  top_match_treatise text,
  top_match_chapter integer,
